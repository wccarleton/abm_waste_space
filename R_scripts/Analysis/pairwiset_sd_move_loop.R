...
for(j in c(0.5,0.1,.01)){
   for(k in c(0,1)){
      for(l in climate_sd_comps){
         x <- subset(pop_lvls_params_df,Move==j & WasteCondition==k & ClimateCondition == l[1])[,"sd"]
         y <- subset(pop_lvls_params_df,Move==j & WasteCondition==k & ClimateCondition == l[2])[,"sd"]
         pval <- t.test(x,
                        y,
                        alternative="less",
                        var.equal=F)$p.value
...
write.table(pt_pvals,file="../Results/Analysis/TTests/pt_pvals_pop_lvls_sd_move_climate.csv",sep=",")
