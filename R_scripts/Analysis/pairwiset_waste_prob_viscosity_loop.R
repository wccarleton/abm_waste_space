...
for(j in c(0.5,0.75,1)){
   for(k in c(0,1)){
      for(l in climate_sd_comps){
         x <- subset(waste_prob_params_df,Viscosity==j & WasteCondition==k & ClimateCondition == l[1])[,"expectation"]
         y <- subset(waste_prob_params_df,Viscosity==j & WasteCondition==k & ClimateCondition == l[2])[,"expectation"]
         pval <- t.test(x,
                        y,
                        alternative="less",
                        var.equal=F)$p.value
...
write.table(pt_pvals,file="../Results/Analysis/TTests/pt_pvals_waste_prob_viscosity_climate_test.csv",sep=",")
