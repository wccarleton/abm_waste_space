...
for(j in exps){
   ...
   matts <- matrix(as.numeric(waste_ts_sorted[which(waste_ts_sorted[,"experiment"]==j),c(5)]),nrow=500,byrow=F)
   matplot(matts,
         type="l",
         lty=1,
         lwd=1,
...
dev.copy(png,file="../Images/ts_waste_prop.png",height=1500,width=1500,units="px",res=150)
dev.off()
