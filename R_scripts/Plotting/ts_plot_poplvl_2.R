...
for(j in exps){
   ...
   matts <- matrix(waste_ts_move[which(waste_ts_move[,"experiment"]==j),c(4)],nrow=500,byrow=F)
   matplot(matts,
         type="l",
         lty=1,
         lwd=1,
...
dev.copy(png,file="../Images/ts_population_levels_2.png",height=1500,width=1500,units="px",res=150)
dev.off()
